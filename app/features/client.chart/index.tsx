...
const CompetitorChart: FC<CompetitorChartProps> = ({ className = "" }) => {
    ...
    return (
      <div className={`w-full h-full relative ${className}`}>
        ...
        <div className="absolute top-0 left-0 z-10">
          <div className="mt-1 inline-flex bg-gray-300 border-2 border-gray-400 animate-pulse">
  ...
  if (!scaleRange || curves.length === 0) {
    return (
      <div className={`flex items-center justify-center h-96 ${className}`}>
        <div className="text-gray-500">No trader data available</div>
      </div>
    );
  }
          ...
          <>
            {selectedTrader && (
              <button
                ...
                className="absolute -left-4 top-1/2 -translate-x-full -translate-y-1/2 cursor-pointer rounded-[2px] bg-black text-white px-2 py-1 text-xs font-light"
              >
                BACK TO ALL
              </button>
            )}
            <span className="text-sm">TOTAL ACCOUNT VALUE</span>
          </>
        ) : (
          <>
            {selectedTrader ? (
              <button
                className="cursor-pointer rounded-[2px] bg-black text-white px-2 py-1 text-xs font-light"
                onClick={() => setSelectedTraderId(null)}
              >
                BACK TO ALL
              </button>
            ) : (
              <span className="text-xs">TOTAL ACCOUNT VALUE</span>
            )}
          </>
        )}
      </div>
